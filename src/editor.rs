...
impl Editor {
            ...
            Key::Ctrl('q') => {
                if self.quit_times > 0 && self.document.is_dirty() {
                    self.status_message = StatusMessage::from(format!(
                            "WARNING! File has unsaved changes. Press Ctrl-Q {} more times to quit.", self.quit_times));
                    self.quit_times -= 1;
                    return Ok(());
                }
    ...
    fn draw_welcome_message(&self) {
        ...
        let mut welcome_message = format!("Basic Editor --version {}", VERSION);
        let width = self.terminal.size().width as usize;
